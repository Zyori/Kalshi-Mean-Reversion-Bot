...
echo "  1. Create /opt/kalshi-mrb/backend/.env with API keys, then set:"
echo "       ENV=prod"
echo "       ADMIN_PASSWORD_HASH=...  SESSION_SECRET=..."
echo "     Generate both with:"
echo "       cd /opt/kalshi-mrb/backend && uv run python scripts/hash_password.py"
echo "  2. Certbot for both hosts:"
echo "       certbot --nginx -d mrb.lutz.bot -d lutz.bot -d www.lutz.bot"
